...
int16_t HAL::Encoder_GetEmulatorButton()
{   
    // Serial.println("GetEmulatorButton");
    if (digitalRead(CONFIG_BUTTON_A) == LOW)
    {
        delayMicroseconds(10);
        while(digitalRead(CONFIG_BUTTON_A) == LOW);
        Serial.println("GetEmulatorButton A");
        return 1;
    }
    ...
    {
        delayMicroseconds(10);
        while(digitalRead(CONFIG_BUTTON_C) == LOW);
        Serial.println("GetEmulatorButton C");
        return 2;
    }
    else
        return 0;
